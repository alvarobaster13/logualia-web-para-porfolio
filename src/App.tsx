...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-bg-warm font-sans text-text-dark pb-20">
      ...
      <header className="bg-logulia-blue text-white py-5 px-10 shadow-md sticky top-0 z-50 flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div className="bg-logulia-yellow text-logulia-blue px-4 py-1.5 font-black text-2xl rounded-full tracking-tighter shadow-sm">
            LOGULIA
          </div>
          <h1 className="text-lg font-light tracking-tight uppercase border-l border-white/20 pl-4 hidden sm:block">
            Logistics | <span className="font-bold">Optimizer System</span>
          </h1>
        </div>
        <div className="flex items-center gap-2 text-[12px] font-medium opacity-80 uppercase tracking-widest bg-white/10 px-3 py-1 rounded-full">
          v2.4.0 Engine Active
        </div>
      </header>

      <main className="max-w-[1400px] mx-auto px-6 mt-6 grid grid-cols-1 lg:grid-cols-[300px_1fr_320px] gap-6 items-start">
        
        {/* Paso 1: Iterator Component (Left Column) */}
        <section className="bg-card-bg rounded-xl border border-border-warm shadow-sm flex flex-col h-[calc(100vh-140px)] sticky top-[100px]">
          ...
          <div className="p-4 bg-[#fafafa] border-t border-border-warm flex justify-between items-center">
            <button className="p-1.5 rounded border border-border-warm bg-white hover:bg-gray-50 transition-colors">
              <ArrowLeft size={16} className="text-text-muted" />
            </button>
            <span className="text-[11px] font-bold text-text-muted">1 de {products.length}</span>
            <button className="p-1.5 rounded border border-border-warm bg-white hover:bg-gray-50 transition-colors">
        ...
        <section className="bg-card-bg rounded-xl border border-border-warm shadow-sm flex flex-col min-h-[calc(100vh-140px)]">
          <div className="p-4 border-b border-border-warm bg-[#fafafa]">
            <h2 className="font-bold text-text-muted uppercase tracking-wider text-[12px] flex items-center gap-2">
            ...
            <AnimatePresence mode="wait">
              {isOptimizing ? (
                ...
                >
                  ...
                  <motion.div className="border-2 border-dotted border-border-warm rounded-xl p-8 flex flex-col items-center justify-center opacity-30 hover:opacity-50 transition-opacity cursor-wait">
        ...
        <section className="bg-card-bg rounded-xl border border-border-warm shadow-sm flex flex-col h-[calc(100vh-140px)] sticky top-[100px]">
          ...
          <div className="p-6 bg-[#f9f9f9] border-t border-border-warm flex flex-col gap-4">
            <div>
              ...
              <div className="text-3xl font-black text-text-dark tracking-tighter">
                {result ? result.totalCost.toFixed(2) : '0.00'}€
              </div>
              <p className="text-[10px] text-text-muted mt-3 leading-relaxed">
                Algoritmo: Heurística Greedy<br />
                Ahorro estimado vs manual: <span className="text-green-600 font-bold">12%</span>
              </p>
            </div>
      ...
      <div className="fixed bottom-6 left-6 flex items-center gap-3">
        <div className="bg-white/80 backdrop-blur-sm border border-border-warm px-4 py-2 rounded-full shadow-sm text-[11px] font-bold text-text-muted uppercase tracking-widest">
            LOGULIA Logistics v2.4.0
        </div>
      </div>
